...
# Authors: Waseem Hussain, Mahender Anumalla and Margaret Catolos
...
# Plot the data
  plot(x, y, 
       main="My First Plot",  # Title of the plot
       xlab="X axis looks good",            # Label for the x-axis
       ylab="Y-axis ok",            # Label for the y-axis
       col="pink",               # Color of the points
       pch=3)  
  help("plot")
...
# Select columns using $ sign
   ...
   myve<- mydata[mydata$Yield>3,] # Extract row that has yield greater than 3500
...
# Read .csv file data into R. File in in Data folder
  waseem<-read.csv("./Data/iris.txt", sep="\t",header = TRUE)
#Check this function to know more help("read.table")
# Export the file as .csv file
  write.csv(waseem, "./Data/waseem.csv",
            row.names=TRUE)
